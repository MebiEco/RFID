...
static volatile bool s_lvgl_ota_pause;
static volatile bool s_lvgl_redraw_after_ota;
...
{
    (void)arg;
    /* TWDT: chi reset khi task con nam trong watchdog. Khi OTA, suspend xoa task
     * khoi WDT — khong duoc van goi reset (loi "task not found" moi 200ms). */
    s_lvgl_wdt_ok = (esp_task_wdt_add(xTaskGetCurrentTaskHandle()) == ESP_OK);

    while (1) {
        while (s_lvgl_ota_pause) {
            vTaskDelay(pdMS_TO_TICKS(200));
        }
        if (s_lvgl_redraw_after_ota) {
            s_lvgl_redraw_after_ota = false;
            /* lcd_ui ve de len buffer LVGL — phai invalidate toan man idle. */
            if (idle_scr) {
                lv_scr_load(idle_scr);
                lv_obj_invalidate(idle_scr);
            }
...
void lv_port_init(esp_lcd_panel_handle_t panel)
{
    ...
    if (!buf1) {
        ESP_LOGE(TAG, "Khong cap phat LVGL draw buffer (%u bytes)", (unsigned)buf_bytes);
        return;
    }
...
{
    ESP_LOGI(TAG, "OTA: tam dung LVGL (cho SPI xong, khong vTaskSuspend)");
    s_lvgl_ota_pause = true;
    vTaskDelay(pdMS_TO_TICKS(20));
    if (s_lvgl_task && s_lvgl_wdt_ok) {
...
{
    ESP_LOGI(TAG, "OTA fail: chay lai LVGL");
    s_lvgl_redraw_after_ota = true;
    s_lvgl_ota_pause = false;
    if (s_lvgl_task && esp_task_wdt_add(s_lvgl_task) == ESP_OK) {
        s_lvgl_wdt_ok = true;
    }
}
