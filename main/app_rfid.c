...
static void rfid_task(void *arg)
{
    ...
    for (;;) {
        ...
        if (g_force_ui_update || last_idle_update_us == 0 ||
            ...
            /* 1.wav chao mung: mac dinh tat (BOARD_AUDIO_PLAY_WELCOME=0) — tranh re luc dang nhap portal. */
            bool ready_to_play_welcome = false;
#if BOARD_AUDIO_PLAY_WELCOME
...
#endif
            } else if (!s_queued_ready_1_wav && !BOARD_AUDIO_PLAY_WELCOME) {
                s_queued_ready_1_wav = true;
                (void)app_ota_take_skip_welcome();
            }
        }
