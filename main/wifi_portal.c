...
{
    if (!host || !sas_key || host[0] == '\0' || sas_key[0] == '\0') {
        return ESP_ERR_INVALID_ARG;
    }
    char norm_host[64] = {0};
    snprintf(norm_host, sizeof(norm_host), "%s", host);
    normalize_azure_host(norm_host);
    if (norm_host[0] == '\0') {
        return ESP_ERR_INVALID_ARG;
    }
    ...
    c.magic = CRED_MAGIC;
    copy_field(c.azure_host, sizeof(c.azure_host), norm_host);
    if (devid && devid[0] != '\0') {
        copy_field(c.azure_dev, sizeof(c.azure_dev), devid);
    }
    copy_field(c.azure_sas, sizeof(c.azure_sas), sas_key);
    esp_err_t err = cred_save(&c);
    if (err == ESP_OK) {
...
static esp_err_t start_httpd(void)
{
    ...
    static const httpd_uri_t u_ping = { .uri = "/api/ping", .method = HTTP_GET, .handler = ping_get_handler, .user_ctx = NULL };
    static const httpd_uri_t u_root = { .uri = "/", .method = HTTP_GET, .handler = portal_root_get_handler, .user_ctx = NULL };
    static const httpd_uri_t u_save_w = { .uri = "/save_wifi", .method = HTTP_POST, .handler = save_wifi_post_handler, .user_ctx = NULL };
    static const httpd_uri_t u_save_a = { .uri = "/save_azure", .method = HTTP_POST, .handler = save_azure_post_handler, .user_ctx = NULL };
    static const httpd_uri_t u_clear_a = { .uri = "/clear_azure", .method = HTTP_POST, .handler = clear_azure_post_handler, .user_ctx = NULL };
    static const httpd_uri_t u_clear = { .uri = "/clear", .method = HTTP_POST, .handler = clear_post_handler, .user_ctx = NULL };
    static const httpd_uri_t u_scans = { .uri = "/scans", .method = HTTP_GET, .handler = scans_get_handler, .user_ctx = NULL };
    static const httpd_uri_t u_scanwifi = { .uri = "/scan", .method = HTTP_GET, .handler = scan_get_handler, .user_ctx = NULL };
    static const httpd_uri_t u_saved_wifis = { .uri = "/saved_wifis", .method = HTTP_GET, .handler = saved_wifis_get_handler, .user_ctx = NULL };
    static const httpd_uri_t u_del_wifi = { .uri = "/del_wifi", .method = HTTP_POST, .handler = del_wifi_post_handler, .user_ctx = NULL };
...
esp_err_t wifi_portal_start(void)
{
    ...
    /* Khoi tao SNTP tren main_task (stack 10KB), tranh tao tren sys_evt (stack nho) */
    sntp_start_or_restart();
...
/* ----------------------------------------------------------------
 * Work Hours (Giờ vào làm & Giờ tan làm/đi về) — NVS keys "work_start", "work_end"
 * ---------------------------------------------------------------- */
#define WORK_START_NVS_KEY "work_start"
#define WORK_END_NVS_KEY   "work_end"
#define WORK_START_DEFAULT "08:30"
#define WORK_END_DEFAULT   "18:00"

void wifi_portal_get_work_hours(char *start_out, size_t start_sz, char *end_out, size_t end_sz)
{
    if (start_out && start_sz > 0) {
        snprintf(start_out, start_sz, "%s", WORK_START_DEFAULT);
        nvs_handle_t h;
        if (nvs_open(NVS_NS, NVS_READONLY, &h) == ESP_OK) {
            size_t len = start_sz;
            if (nvs_get_str(h, WORK_START_NVS_KEY, start_out, &len) == ESP_OK && start_out[0] != '\0') {
                /* ok */
            }
            nvs_close(h);
        }
    ...
    if (end_out && end_sz > 0) {
        snprintf(end_out, end_sz, "%s", WORK_END_DEFAULT);
        nvs_handle_t h;
        if (nvs_open(NVS_NS, NVS_READONLY, &h) == ESP_OK) {
            size_t len = end_sz;
            if (nvs_get_str(h, WORK_END_NVS_KEY, end_out, &len) == ESP_OK && end_out[0] != '\0') {
                /* ok */
            }
            nvs_close(h);
        }
...
{
    if (!start_str || !end_str) return ESP_ERR_INVALID_ARG;
    int sh = 0, sm = 0, eh = 0, em = 0;
    if (sscanf(start_str, "%d:%d", &sh, &sm) != 2 || sh < 0 || sh > 23 || sm < 0 || sm > 59) {
        return ESP_ERR_INVALID_ARG;
    }
    if (sscanf(end_str, "%d:%d", &eh, &em) != 2 || eh < 0 || eh > 23 || em < 0 || em > 59) {
        return ESP_ERR_INVALID_ARG;
    }
    char s_tmp[8], e_tmp[8];
    snprintf(s_tmp, sizeof(s_tmp), "%02d:%02d", sh, sm);
    snprintf(e_tmp, sizeof(e_tmp), "%02d:%02d", eh, em);

    nvs_handle_t h;
    esp_err_t err = nvs_open(NVS_NS, NVS_READWRITE, &h);
    if (err != ESP_OK) return err;

    err = nvs_set_str(h, WORK_START_NVS_KEY, s_tmp);
    if (err == ESP_OK) err = nvs_set_str(h, WORK_END_NVS_KEY, e_tmp);
    if (err == ESP_OK) err = nvs_commit(h);
    nvs_close(h);
    return err;
}
...
{
    char s_str[8] = {0}, e_str[8] = {0};
    wifi_portal_get_work_hours(s_str, sizeof(s_str), e_str, sizeof(e_str));
    int sh = 8, sm = 30, eh = 18, em = 0;
    sscanf(s_str, "%d:%d", &sh, &sm);
    sscanf(e_str, "%d:%d", &eh, &em);
    if (start_min) *start_min = sh * 60 + sm;
    if (end_min) *end_min = eh * 60 + em;
}
